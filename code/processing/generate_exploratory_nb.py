...
for data_directory in dir_list:
    ...
    # Notebook to make
    to_path = os.path.join(processing_path, timestamp, data_directory+".ipynb") 
    
    # Copy file (overwrite if necessary)
    from_path = os.path.normpath(from_path)
    to_path = os.path.normpath(to_path)
    
    # Delete old version it already exists
    ...
    run_jnb(to_path, return_mode='parametrised_only', data_path=data_directory)
    
    # TODO: replace empty template with the contents of _run_jnb?
    

#%%
# sleep_csv_generator_template_name = ''
# sleep_csv_generator_path= os.path.join(templates_path, ___)
